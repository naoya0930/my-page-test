...
> **更新日**: 2026年6月28日  
> **ベース**: `docs/system_design.md`（要件定義書 v2.3）+ `docs/screen_spec.md`（画面仕様書 v2.4）

このドキュメントは `system_design.md` をベースに、実装対象の画面とAPIを一覧化したものです。spec2（統計・タイムトラベル）、spec3（サインアップ・dayX編集）、spec4（規約同意・完遂演出・UI改善）、spec5（本日表示・過去週UI整理・達成セレブレーション）までの内容を反映しています。
...
- 7日間のドットグリッド（Day1〜Day7、文字数表示、クリック可能）＋**今週の達成日数「X/7」テキスト併記（spec4）**＋**現在週表示時は本日の要素をハイライト（バッジ「今日」＋リング、spec5）**
- **過去週UIの整理（spec5）**: 「(閲覧のみ)」テキスト削除（操作制御は維持）、「過去の記録を閲覧できます」を `/statistics` へのリンク化、「モーニングページ」の「未完了」ラベル削除
- **達成セレブレーション演出（spec5）**: モーニングページ 7/7 または アーティストデート完了（OR）で該当カードを薄ゴールド背景＋豪華アイコンに（控えめ・状態ベースで毎回適用）
...
- 日次アクティビティヒートマップ（12週×7日＝84マス、GitHub草風、ホバーで詳細）＋**本日セルの強調（枠線アクセント＋ツールチップ「今日」、spec5）**
