...
- [ ] `src/main.tsx` に `/statistics` のルートを追加する
- [ ] 統計画面への認証ガードを設定する

### 8.2 バックエンドAPI拡張

- [ ] `GET /api/progress` を拡張し、`week_number` パラメータを受け取れるようにする
- [ ] `GET /api/progress?week_number=N` で指定週の進捗・モーニングページ7日間分・アーティストデート状態を返すようにする
- [ ] `GET /api/statistics` エンドポイントを新設する
  - 12週間総合サマリー（総執筆日数、累計総文字数、アーティストデート達成率）
  - 週ごとの文字数集計（GROUP BY week_number）
  - 日次アクティビティデータ（84日分）
  - アーティストデート週次達成状況
- [ ] `POST /api/morning-pages` 保存時に `Progress.morning_page_done` を自動更新するロジックを実装する
- [ ] `POST /api/artist-dates` 保存時に `Progress.artist_date_done` を自動更新するロジックを実装する

### 8.3 ユーザーホーム画面の大幅改修

#### 8.3.1 週次タイムトラベル機能

- [ ] 週次表示に左右スワイプ（またはボタン「＜」「＞」）機能を実装する
- [ ] 現在の経過週を初期表示として算出する
- [ ] 第1週〜第12週までの切り替えを可能にする
- [ ] 表示中の週番号を状態管理（useState）する

#### 8.3.2 視覚的演出（タイムトラベル中の表現）

- [ ] **過去週表示時**: セピア/モノトーン（グレースケール）のスタイル適用
- [ ] **過去週表示時**: 入力ボタンを非活性化（閲覧のみモード）
- [ ] **未来週表示時**: 半透明オーバーレイ/ソフトブルーグラデーション適用
- [ ] **未来週表示時**: タスクをロック状態で表示
- [ ] **現在週表示時**: 通常ブランドカラー（高コントラスト）で表示

#### 8.3.3 モーニングページ日次ステータス表示

- [ ] 7日間のドットグリッド（Day 1〜Day 7）をデザイン・実装する
- [ ] 実行済みの日: 鮮やかな成功カラー（ターコイズグリーン、チェックマーク）で表示
- [ ] 未実行の日: グレーアウト/破線枠で表示
- [ ] 各ドットの下部に文字数を小さく表示する
- [ ] 当日のドットまたはボタンから `/morning-page` への遷移を実装

#### 8.3.4 バグ修正: 編集後のステータス同期

- [ ] モーニングページ保存後、フロントエンドのキャッシュを無効化（Invalidate）する
- [ ] ホーム画面マウント時に最新のDB状態を取得するよう修正する
- [ ] Progress APIの再フェッチ（Refetch）ロジックを実装する
- [ ] 保存成功時の状態同期を遅延なく行うよう改善する

#### 8.3.5 共通ナビゲーション

- [ ] ヘッダーに `/home` と `/statistics` への相互遷移リンクを追加する

### 8.4 アーティストデート画面の改修

- [ ] 保存成功時に自動的に `/home` へリダイレクトする機能を実装する
- [ ] 保存失敗時は画面遷移せず、エラーメッセージを表示して再試行を促す

### 8.5 統計画面の新規実装

- [ ] `src/pages/StatisticsPage.tsx` を新規作成する

#### 8.5.1 12週間総合サマリーカード

- [ ] モーニングページの総執筆日数を表示（例: 52日 / 84日中）
- [ ] 累計総文字数を表示（例: 68,500 文字）
- [ ] アーティストデート累計達成率を表示（例: 8週 / 12週中）

#### 8.5.2 12週間の執筆文字数推移グラフ

- [ ] グラフライブラリ（Recharts / Chart.js）を選定・インストールする
- [ ] 横軸: Week 1〜Week 12、縦軸: 各週の合計/平均文字数の棒グラフを実装する
- [ ] 未到達の未来週は「Pending」または非表示として扱う

#### 8.5.3 日次アクティビティヒートマップ

- [ ] 12週間×7日間（84マス）のマトリクスUIを実装する
- [ ] GitHub草グラフ風のデザインを適用する
- [ ] モーニングページ実行日は文字数に応じて濃い色で表示
- [ ] 未実行日は空白/薄いグレーで表示
- [ ] マウスホバーで詳細（日付・文字数）を表示する

#### 8.5.4 アーティストデート軌跡表示

- [ ] 第1週〜第12週の `went_out` と `excited` の達成状況を一覧表示する
- [ ] アイコンのオン/オフで視覚化する

### 8.6 共通コンポーネント改修

- [ ] `src/components/Layout.tsx` に統計画面へのリンクを追加する
- [ ] ヘッダーのナビゲーションメニューを拡張する

### 8.7 テスト・検証

- [ ] 週次タイムトラベル機能の動作確認（過去・現在・未来）
- [ ] 視覚的演出（色調変化）の確認
- [ ] モーニングページ日次ステータス表示の確認
- [ ] 統計画面のデータ表示確認（サマリー、グラフ、ヒートマップ）
- [ ] アーティストデート保存後の自動リダイレクト確認
- [ ] ステータス同期バグの修正確認
- [ ] 新規APIエンドポイント（`/api/statistics`、拡張された `/api/progress`）の動作確認
- [ ] 統合テストスクリプトの更新と実行
