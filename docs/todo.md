...
- [ ] `src/pages/LoginPage.tsx` に「ログイン」/「新規アカウント作成」のモード切り替えタブを実装する
- [ ] アカウント作成フォーム（メール / パスワード / パスワード確認）を追加する
- [ ] クライアントバリデーション（メール形式、パスワード一致）を実装する
- [ ] Supabase Auth のサインアップAPIを呼び出す処理を実装する
- [ ] サインアップ成功時に確認メール案内表示、または自動ログインして `/home` へ遷移する
- [ ] サインアップ時のエラー（「既に登録されているメールアドレスです」等）を表示する

### 9.2 ユーザーホーム画面の改修

- [ ] 全体進捗インジケーター「Day XX / 84」を画面上部に表示する（経過日数を算出）
- [ ] 7日間のドット（`dayX`）をクリック可能なボタンとして実装する
- [ ] dayX クリックで `/morning-page?date=YYYY-MM-DD` へ遷移する
- [ ] 過去日クリック時に確認ダイアログ（「過去のページです。編集を開始してよろしいですか？」）を表示し、OK時のみ遷移する
- [ ] 未来日のドットはロック状態とし、クリック遷移を不可にする
- [ ] **日次ステータス同期**: モーニングページを保存（書き込み）した日は日次ステータス（色 / チェック）を「完了」に更新する（保存成功時にキャッシュ無効化 / 再フェッチで反映）

### 9.3 モーニングページ画面の改修

- [ ] `?date=YYYY-MM-DD` クエリを読み取り、対象日のデータを取得・表示する（未指定時は当日）
- [ ] タイトルに対象日（「Day X のモーニングページ」/ 日付）を表示する
- [ ] 保存成功（`200 OK`）時に `/home` へ自動リダイレクトする
- [ ] 保存失敗時は遷移せずエラーメッセージを表示して再試行を促す
